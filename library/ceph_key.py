...
options:
    ...
    state:
        description:
            - If 'present' is used a secret is provided the module
            ...
            If list, the module will list all the keys and will
            return a json output.
            If 'update' is used, the module will the capabilities **only**
            of a given keyring
...
def generate_ceph_authtool_cmd(cluster, name, secret, caps, containerized=None):
    '''
    Generate 'ceph-authtool' command line to execute
    '''

    file_destination = os.path.join("/etc/ceph/" + cluster + "." + name + ".keyring")
...
def create_key(module, result, cluster, name, secret, caps, containerized=None):
    '''
    Create a CephX key
    '''

    file_path = os.path.join("/etc/ceph/" + cluster + "." + name + ".keyring")
